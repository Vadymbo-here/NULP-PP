...
public class JsonWorker {
    ...
    public static String getJsonStringF() {
        ...
        String jsonString = "";
        try {
            fr = new FileReader("D:\\Code\\Proggrams\\PP_labki1\\NULP_PP\\COMPLEX\\deposits2\\src\\database2.json");
            int i;
            while ((i = fr.read()) != -1) {
                jsonString += ((char) i);
            }
    ...
    public static void writeToDatabase(String where, JSONObject obj) throws IOException {
        ...
        PrintWriter printRecord = new PrintWriter(new BufferedWriter(new FileWriter("src\\database2.json", false)));
    ...
    public static String[] printAllDepositsArray(int userID) {
        ArrayList<Deposit> deps = getDepList();
        ArrayList<Bank> banks = getBankList();
        String[] res = new String[25];
        if (deps.isEmpty()) {// error
            res[0] = "Looks like there is no deposits.";
            return res;
        }
        // System.out.println("Your active deposits:");
        ...
        if (!flag) {
            res[0] = "Looks like you have no active deposits. Maybe it's time to make one?";
            return res;
        }
        return res;
    }
